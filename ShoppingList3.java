 ...
 * This provides simple operations like add, delete, show, sort, save and exit.
 * 
 * @author Jeff Light
 * @author Alex May (modified by)
 * Modified by Karla Cortez
 */
		        ...
		        do {
		            ...
		            switch (choice) {
		                ...
		                case "4": // Sort the items in the Shopping List
		                    ...
		                    break;
		                case "5": 
		                	saveList(sIn,  shoppingList, "ShoppingList.txt");
		                	break;
		            ...
		            try {
		                ...
		                if (item.equals(""))
		                    ...
		                    if (!item.contains(":"))
		                        ...
		                        String itemName = item.substring(0, separator).trim();
		                        int itemAm = 0;
		                        
		                        String amount = item.substring(separator + 1).trim();
		                        itemAm = Integer.parseInt(amount);

		                        shoppingList.add(itemName + ":" + itemAm);
		                        System.out.println(String.format("\n'%s:%s' has been added to the Shopping List..", itemName, itemAm));
		                        newItems++;
		                    }
		                }
		            } catch (Exception e) {
		        ...
		        do {
		            try {
		                ...
		                if (item.equals(""))
		                    ...
		                    nextItem = true;
		                    boolean exitsItem = shoppingList.remove(item);
		                    if (!exitsItem) {
		                        System.out.println(String.format("\nInvalid Response! '%s' is NOT an item in the list.", item));
		                        showItems(shoppingList);
		                    }
		                    else {
		                        System.out.println(String.format("\n'%s' has been deleted from the Shopping List..", item));
		                        newItems++;
		                    }
		                }
		            } catch (Exception e) {
		    ...
		    */
		    ...
		    public static void showItems(ArrayList<String> shoppingList) {
		        try {
		            ...
		            System.out.println("-----------------------");
		            for (String itm : shoppingList) {
		                int separator = itm.indexOf(":");
		                String itemName = itm.substring(0, separator).trim();
		                String itemAm = itm.substring(separator + 1).trim();
		                System.out.println(String.format("%s\t%s", itemName, itemAm));
		            }
		            System.out.println("-----------------------");
		        } catch(Exception e) {
 ...
 public static void saveList (Scanner in, ArrayList<String> shoppingList, String fileName) {
	 ...
	 boolean valid = false; 
		if (userInput.equalsIgnoreCase("n")) {
			...
			try {
				FileWriter shoppingListFile = new FileWriter(fileName);
				for (int i= 0; i < shoppingList.size(); i++) {
					shoppingListFile.write(shoppingList.get(i));
					if (i != shoppingList.size() - 1) {
						shoppingListFile.write("\n");
					}
				}
