 ...
 * The application will allow users to select from a menu of choices such as 
Find the Sum, Find the Average and Find the Biggest.
 * Presents a menu to the user and calls the playMath() method with the 
associated argument.
 * 
...
/**
 * Returns void.
 * Allows the user to enter in a series of numbers (whole or double) and 
loads them into an ArrayList<Double> called numList.
 ...
 * @param game A String indicating the game the user chose to play.  Will 
= "sum", "average", or "biggest".
 * @return void
 */
...
double num = 0;
//Double variable to hold the user input after converted to a number
        double result = 0;
//The value returned from the method called.
...
//Process every number in the ArrayList
result = result + numList.get(i);
System.out.printf("%,-15.2f%,.2f\n",numList.get(i), result);
...
System.out.printf("%,-15.2f%,.2f\n", numList.get(i), (sum/(i+1)));
//Print the running average
}//end of for (int i = 0; i < numList.size(); i++)
result = sum / numList.size();
...
double result = 0;
//Initialize the result
System.out.printf("%-12s%s\n","Number", "Running Biggest");
//Print the header
for (int i = 0; i < numList.size(); i++) {
//Process every number in the ArrayList
if(numList.get(i) > result) {
result = numList.get(i);
//If the number processed is bigger than the current biggest, it is the new biggest
